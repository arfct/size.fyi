...
Catalog data lives in `data/devices/<category>/<slug>.json` — **one device per file**, dimensions in millimetres (height × width × depth) from manufacturer specs. Copy [`data/devices/device.template.json`](data/devices/device.template.json), fill it in, and run `npm run validate`.

See **[CONTRIBUTING.md](CONTRIBUTING.md)** for the full schema (rounding, multi-state foldables, 3D models) and pull-request workflow.

The compiled catalog `public/devices.json` is generated by the build (`npm run build`/`dev`) and is git-ignored — never edit it by hand.
...
1. **Edit catalog:** Add or edit `data/devices/<category>/<slug>.json` files
