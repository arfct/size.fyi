...
export interface DeviceState {
  ...
  h: number; w: number; d: number;            // mm, for this state
  radius?: number; radiusAxis?: RadiusAxis;
  screen?: Screen;
...
export interface Device {
  ...
  h: number; w: number; d: number;            // mm — the default state's dims for multi-state devices
...
export interface ResolvedDims {
  ...
  radius?: number; radiusAxis?: RadiusAxis; screen?: Screen; seam?: boolean;
}
...
export function deviceDims(device: Device, state?: string): ResolvedDims {
  ...
  if (s) return { h: s.h, w: s.w, d: s.d, radius: s.radius, radiusAxis: s.radiusAxis, screen: s.screen, seam: s.seam };
  return { h: device.h, w: device.w, d: device.d, radius: device.radius, radiusAxis: device.radiusAxis, screen: device.screen };
}
