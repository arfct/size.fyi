...
if (!srcDir) { console.error('usage: build-models.mjs <src-dir>'); process.exit(1); }
...
for (const file of (await readdir(srcDir)).filter((f) => f.endsWith('.glb')).sort()) {
  ...
  console.log(`${file}: ${Math.round(before / 1000)}k → ${Math.round(triCount(doc) / 1000)}k tris, ${(size / 1024).toFixed(0)} KB`);
}
